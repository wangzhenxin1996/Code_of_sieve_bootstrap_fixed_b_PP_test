...
dts=[1 1 2 3];
type='bt';
size_zas=zeros(5,12,num_bs);
size_zts=zeros(5,12,num_bs);
...
for h=1:num_bs
    ...
    for e=1:3 
        ...
        for f=1:5  
            rho=rhos(:,f)';
            phi=phis(:,f)';
            % PP with OLS_detrend or PP(fb) with one-step OLS detrend or PP(fb) with two-step OLS detrend or PP^b(fb) with GLS detrend
            for g=1:4 
                pp=pps(g);
                dt=dts(g);
...
save('SizeCurveforT_200withDifferentB','size_zas','size_zts');
